...
async def train(
    ...
    with open('./memories/text_memories.yaml', 'r', encoding='utf-8') as f:
        text_memories = yaml.safe_load(f)
    
    memories = [rule.get('description', '') for rule in text_memories.get('memory_rules', [])]
    ...
    with open('./memories/sql_examples.yaml', 'r', encoding='utf-8') as f:
        sql_examples = yaml.safe_load(f)

    examples = [{'question': rule.get('question', ""), 'sql': rule.get('sql', '')} for rule in sql_examples.get('examples', [])]
    for ex in examples:
