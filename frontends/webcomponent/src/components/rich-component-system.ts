...
export class DataFrameComponentRenderer extends BaseComponentRenderer {
  render(component: RichComponent): HTMLElement {
    ...
    if (searchable || exportable || filterable) {
      actionsHTML = `
        <div class="dataframe-actions">
          ...
          ` : ''}
          <div class="dataframe-view-toggle">
            <button class="view-toggle-btn active" data-view="table" title="Table view">Table</button>
            <button class="view-toggle-btn" data-view="json" title="JSON view">JSON</button>
          </div>
          ${exportable ? `
    ...
    // Build JSON view HTML
    const jsonHTML = `
      <div class="dataframe-json-container" style="display: none;">
        <pre class="dataframe-json"><code>${this.escapeHtml(JSON.stringify(displayedData, null, 2))}</code></pre>
      </div>
    `;
    ...
    const tableContainer = container.querySelector('.dataframe-table-container, .dataframe-empty') as HTMLElement;
    const jsonContainer = container.querySelector('.dataframe-json-container') as HTMLElement;
    toggleBtns.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (jsonContainer) jsonContainer.style.display = view === 'json' ? '' : 'none';
      });
    });
